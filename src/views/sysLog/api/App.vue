<template>
  <div v-loading="loading" class="ofa-container column">
    <div class="header">
      <div class="title-box">
        <span class="title ofa-text-title">Api日志<span class="sub-title" style="margin-left: 10px;">共有<label
    ...
    <el-table :data="list" size="small" class="ofa-table">
      ...
      <el-table-column prop="TimeSpan" label="耗时" min-width="160"></el-table-column>
      ...
      <el-table-column prop="StatusCode" label="状态码" width="100">
        <template slot-scope="scope">
          <label
            :class="{ 'ofa-text-success' : scope.row.StatusCode === 200, 'ofa-text-danger' : scope.row.StatusCode === 500 }">{{ scope.row.StatusCode }}</label>
        </template>
      </el-table-column>
